...
class cclientes extends cTable {
	...
	function __construct() {
		...
		// RazonSocial
		$this->RazonSocial = new cField('clientes', 'clientes', 'x_RazonSocial', 'RazonSocial', '`RazonSocial`', '`RazonSocial`', 200, -1, FALSE, '`RazonSocial`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['RazonSocial'] = &$this->RazonSocial;

		// ImagenLogo
		$this->ImagenLogo = new cField('clientes', 'clientes', 'x_ImagenLogo', 'ImagenLogo', '`ImagenLogo`', '`ImagenLogo`', 205, -1, TRUE, '`ImagenLogo`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['ImagenLogo'] = &$this->ImagenLogo;

		// Comentario
		$this->Comentario = new cField('clientes', 'clientes', 'x_Comentario', 'Comentario', '`Comentario`', '`Comentario`', 201, -1, FALSE, '`Comentario`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['Comentario'] = &$this->Comentario;

		// Estado
		$this->Estado = new cField('clientes', 'clientes', 'x_Estado', 'Estado', '`Estado`', '`Estado`', 200, -1, FALSE, '`Estado`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function getSqlOrderBy() { // Order By
		return ($this->_SqlOrderBy <> "") ? $this->_SqlOrderBy : "`RazonSocial` ASC,`Estado` ASC";
	}
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->ImagenLogo->Exportable) $Doc->ExportCaption($this->ImagenLogo);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->ImagenLogo->Exportable) $Doc->ExportField($this->ImagenLogo);
