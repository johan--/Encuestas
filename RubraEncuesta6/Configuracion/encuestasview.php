...
$encuestas_view = NULL; // Initialize page object first

class cencuestas_view extends cencuestas {
	...
	// Page object name
	var $PageObjName = 'encuestas_view';
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					...
					if ($this->TotalRecs <= 0) { // No record found
						...
						$this->Page_Terminate("encuestaslist.php"); // Return to list page
					...
					if (!$bMatchRecord) {
						if ($this->getSuccessMessage() == "" && $this->getFailureMessage() == "")
							$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record message
						$sReturnUrl = "encuestaslist.php"; // No matching record, return to list
					} else {
			...
			$sReturnUrl = "encuestaslist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($encuestas_view)) $encuestas_view = new cencuestas_view();

// Page init
$encuestas_view->Page_Init();

// Page main
$encuestas_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$encuestas_view->Page_Render();
?>
...
// Page object
var encuestas_view = new ew_Page("encuestas_view");
encuestas_view.PageID = "view"; // Page ID
var EW_PAGE_ID = encuestas_view.PageID; // For backward compatibility

// Form object
var fencuestasview = new ew_Form("fencuestasview");

// Form_CustomValidate event
fencuestasview.Form_CustomValidate = 
...
fencuestasview.ValidateRequired = true;
<?php } else { ?>
fencuestasview.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fencuestasview.Lists["x_idEvento"] = {"LinkField":"x_Id","Ajax":null,"AutoFill":false,"DisplayFields":["x_Nombre","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php } ?>
<?php $encuestas_view->ExportOptions->Render("body") ?>
<?php
	foreach ($encuestas_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php if ($encuestas->Export == "") { ?>
...
<?php } ?>
<?php $encuestas_view->ShowPageHeader(); ?>
<?php
$encuestas_view->ShowMessage();
?>
<?php if ($encuestas->Export == "") { ?>
<form name="ewPagerForm" class="form-inline ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($encuestas_view->Pager)) $encuestas_view->Pager = new cPrevNextPager($encuestas_view->StartRec, $encuestas_view->DisplayRecs, $encuestas_view->TotalRecs) ?>
<?php if ($encuestas_view->Pager->RecordCount > 0) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($encuestas_view->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $encuestas_view->PageUrl() ?>start=<?php echo $encuestas_view->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($encuestas_view->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $encuestas_view->PageUrl() ?>start=<?php echo $encuestas_view->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $encuestas_view->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($encuestas_view->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $encuestas_view->PageUrl() ?>start=<?php echo $encuestas_view->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($encuestas_view->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $encuestas_view->PageUrl() ?>start=<?php echo $encuestas_view->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $encuestas_view->Pager->PageCount ?></span>
</div>
<?php } ?>
<div class="clearfix"></div>
</form>
<?php } ?>
<form name="fencuestasview" id="fencuestasview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($encuestas_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $encuestas_view->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="encuestas">
<table class="table table-bordered table-striped ewViewTable">
<?php if ($encuestas->idEvento->Visible) { // idEvento ?>
	<tr id="r_idEvento">
		<td><span id="elh_encuestas_idEvento"><?php echo $encuestas->idEvento->FldCaption() ?></span></td>
		<td<?php echo $encuestas->idEvento->CellAttributes() ?>>
<span id="el_encuestas_idEvento" class="form-group">
<span<?php echo $encuestas->idEvento->ViewAttributes() ?>>
<?php echo $encuestas->idEvento->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($encuestas->objetivo->Visible) { // objetivo ?>
	<tr id="r_objetivo">
		<td><span id="elh_encuestas_objetivo"><?php echo $encuestas->objetivo->FldCaption() ?></span></td>
		<td<?php echo $encuestas->objetivo->CellAttributes() ?>>
<span id="el_encuestas_objetivo" class="form-group">
<span<?php echo $encuestas->objetivo->ViewAttributes() ?>>
<?php echo $encuestas->objetivo->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($encuestas->fechaInicio->Visible) { // fechaInicio ?>
	<tr id="r_fechaInicio">
		<td><span id="elh_encuestas_fechaInicio"><?php echo $encuestas->fechaInicio->FldCaption() ?></span></td>
		<td<?php echo $encuestas->fechaInicio->CellAttributes() ?>>
<span id="el_encuestas_fechaInicio" class="form-group">
<span<?php echo $encuestas->fechaInicio->ViewAttributes() ?>>
<?php echo $encuestas->fechaInicio->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($encuestas->fechaFin->Visible) { // fechaFin ?>
	<tr id="r_fechaFin">
		<td><span id="elh_encuestas_fechaFin"><?php echo $encuestas->fechaFin->FldCaption() ?></span></td>
		<td<?php echo $encuestas->fechaFin->CellAttributes() ?>>
<span id="el_encuestas_fechaFin" class="form-group">
<span<?php echo $encuestas->fechaFin->ViewAttributes() ?>>
<?php echo $encuestas->fechaFin->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($encuestas->logo_encuesta->Visible) { // logo_encuesta ?>
	<tr id="r_logo_encuesta">
		<td><span id="elh_encuestas_logo_encuesta"><?php echo $encuestas->logo_encuesta->FldCaption() ?></span></td>
		<td<?php echo $encuestas->logo_encuesta->CellAttributes() ?>>
<span id="el_encuestas_logo_encuesta" class="form-group">
<span<?php echo $encuestas->logo_encuesta->ViewAttributes() ?>>
<?php if ($encuestas->logo_encuesta->LinkAttributes() <> "") { ?>
<?php if (!empty($encuestas->logo_encuesta->Upload->DbValue)) { ?>
<a<?php echo $encuestas->logo_encuesta->LinkAttributes() ?>><?php echo $encuestas->logo_encuesta->ViewValue ?></a>
<?php } elseif (!in_array($encuestas->CurrentAction, array("I", "edit", "gridedit"))) { ?>	
&nbsp;
<?php } ?>
<?php } else { ?>
<?php if (!empty($encuestas->logo_encuesta->Upload->DbValue)) { ?>
<?php echo $encuestas->logo_encuesta->ViewValue ?>
<?php } elseif (!in_array($encuestas->CurrentAction, array("I", "edit", "gridedit"))) { ?>	
&nbsp;
<?php } ?>
<?php } ?>
...
<?php if ($encuestas->rtaMultiple->Visible) { // rtaMultiple ?>
	<tr id="r_rtaMultiple">
		<td><span id="elh_encuestas_rtaMultiple"><?php echo $encuestas->rtaMultiple->FldCaption() ?></span></td>
		<td<?php echo $encuestas->rtaMultiple->CellAttributes() ?>>
<span id="el_encuestas_rtaMultiple" class="form-group">
<span<?php echo $encuestas->rtaMultiple->ViewAttributes() ?>>
<?php echo $encuestas->rtaMultiple->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($encuestas->estado->Visible) { // estado ?>
	<tr id="r_estado">
		<td><span id="elh_encuestas_estado"><?php echo $encuestas->estado->FldCaption() ?></span></td>
		<td<?php echo $encuestas->estado->CellAttributes() ?>>
<span id="el_encuestas_estado" class="form-group">
<span<?php echo $encuestas->estado->ViewAttributes() ?>>
<?php echo $encuestas->estado->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
