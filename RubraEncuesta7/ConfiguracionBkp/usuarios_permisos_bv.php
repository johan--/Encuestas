...
$usuarios_permisos_blobview = NULL; // Initialize page object first

class cusuarios_permisos_blobview extends cusuarios {
	...
	// Page object name
	var $PageObjName = 'usuarios_permisos_blobview';
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel(CurrentProjectID() . 'usuarios');
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('usuarios', 'x_permisos');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (!EW_DEBUG_ENABLED && ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('permisos');
...
// Create page object
if (!isset($usuarios_permisos_blobview)) $usuarios_permisos_blobview = new cusuarios_permisos_blobview();

// Page init
$usuarios_permisos_blobview->Page_Init();

// Page main
$usuarios_permisos_blobview->Page_Main();
?>
<?php
$usuarios_permisos_blobview->Page_Terminate();
?>
